  ...
  void _handleScaleUpdate(ScaleUpdateDetails details) {
    ...
    if (_action == _CropAction.cropping) {
      ...
      if (_handle == _CropHandleSide.topLeft) {
        _updateArea(left: dx, top: dy, cropHandleSide: _handle);
      } else if (_handle == _CropHandleSide.topRight) {
        _updateArea(top: dy, right: dx, cropHandleSide: _handle);
      } else if (_handle == _CropHandleSide.bottomLeft) {
        _updateArea(left: dx, bottom: dy, cropHandleSide: _handle);
      } else if (_handle == _CropHandleSide.bottomRight) {
        _updateArea(right: dx, bottom: dy, cropHandleSide: _handle);
      }
      ...
      setState(() {
        if ((widget.rotationDegree.remainder(360)) == 0) {
          _view = _view.translate(
            delta.dx / (image.width * _scale * _ratio),
            delta.dy / (image.height * _scale * _ratio),
          );
        } else if ((widget.rotationDegree.remainder(360)) == 90) {
          _view = _view.translate(
            delta.dy / (image.width * _scale * _ratio),
            -delta.dx / (image.height * _scale * _ratio),
          );
        } else if ((widget.rotationDegree.remainder(360)) == 180) {
          _view = _view.translate(
            -delta.dx / (image.width * _scale * _ratio),
            -delta.dy / (image.height * _scale * _ratio),
          );
        } else if ((widget.rotationDegree.remainder(360)) == 270) {
          _view = _view.translate(
            -delta.dy / (image.height * _scale * _ratio),
            delta.dx / (image.width * _scale * _ratio),
          );
